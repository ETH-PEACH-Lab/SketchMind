...
type MCQ = {
  ...
  const MCQS: MCQ[] = [
    {
      id: 'q1',
      promptEN: 'Q1 · What is the core iterative logic for reversing a linked list?',
      promptZH: 'Q1 · 迭代反转链表的核心逻辑是什么？',
      options: [
        { key: 'A', textEN: 'Always jump to the last node and set it as the new head', textZH: '总是跳到最后一个节点并把它设为新头结点' },
        { key: 'B', textEN: 'Sequentially redirect each node’s next pointer to its previous node while traversing', textZH: '遍历时依次把每个节点的 next 指向其前驱节点' },
        { key: 'C', textEN: 'Copy nodes into an array and rebuild the list in reverse', textZH: '把节点复制到数组中再逆序重建链表' },
        { key: 'D', textEN: 'Swap values of symmetric nodes until the middle', textZH: '交换首尾对称位置的值直到中间' },
      ],
    },
    {
      id: 'q2',
      promptEN: 'Q2 · True or False: We must store the next node before overwriting pointers during iteration.',
      promptZH: 'Q2 · 判断：在迭代过程中，重写指针之前必须先保存后继节点。',
      options: [
    ...
    },
    {
      id: 'q4',
      promptEN:
        'Q4 · Given head = [1,2,3], when the first loop iteration starts, which pointer update should happen first?',
      promptZH:
        'Q4 · 给定 head = [1,2,3]，在第一次循环迭代开始时，下面哪一步应当先进行？',
      options: [
        { key: 'A', textEN: 'Move curr forward to curr->next', textZH: '把 curr 前移到 curr->next' },
        { key: 'B', textEN: 'Set curr->next = prev', textZH: '先令 curr->next = prev' },
        { key: 'C', textEN: 'Save curr->next into a temporary variable', textZH: '先把 curr->next 保存到临时变量中' },
        { key: 'D', textEN: 'Set prev to nullptr again', textZH: '把 prev 再次设为 nullptr' },
      ],
    },
  ];
  ...
  const ORDERING_Q: OrderingQ = {
    id: 'q3',
    promptEN: 'Q3 · Arrange the steps into the correct iterative process for reversal.',
    promptZH: 'Q3 · 将以下步骤按正确的迭代反转流程排序。',
    stepsEN: [
  ...
  const FILL_Q: FillBlankQ = {
    ...
    promptZH: 'Q5 · 填空（C++）：',
    blanks: [
      { id: 'b1', placeholder: 'init prev' },        // 期望答案：nullptr
      { id: 'b2', placeholder: 'final return' },     // 期望答案：prev
    ],
    footerEN:
      'ListNode* prev = ___________;\nListNode* curr = head;\nwhile (curr) {\n  ListNode* next = curr->next;\n  curr->next = prev;\n  prev = curr;\n  curr = next;\n}\nreturn ___________;',
    footerZH:
      'ListNode* prev = ___________;\nListNode* curr = head;\nwhile (curr) {\n  ListNode* next = curr->next;\n  curr->next = prev;\n  prev = curr;\n  curr = next;\n}\nreturn ___________;',
  };
...
<Box sx={{ p: 3, pt: 0 }}>
  ...
  {zh ? '迭代 · 反转链表' : 'Iteration · Reverse Linked List'}
</Typography>
...
<Paper variant="outlined" sx={{ p: 2, mb: 2 }}>
  <Typography sx={{ mb: 1.5 }}>
    {zh
      ? '给定单链表的头节点 head，请反转链表并返回反转后的头节点。请用迭代思路回答以下题目。'
      : 'Given the head of a singly linked list, reverse the list and return the reversed head. Answer the following using the iterative approach.'}
  </Typography>
  <Divider />
</Paper>
