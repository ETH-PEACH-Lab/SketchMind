...
export default function StoryPlayer({
  ...
  useEffect(() => {
    ...
    const ch = (containerRef?.current as any)?.clientHeight ?? (typeof window !== 'undefined' ? window.innerHeight : 800)
    // 根据左侧面板状态调整水平位置
    const horizontalMultiplier = 0.4 // 卡片居中显示
    const next = { x: cw * horizontalMultiplier, y: ch * 0.7 } // 水平位置，底部上方留空
    setPosition(clampToContainer(next.x, next.y))
    // 初始测量后，用实际高度做一次精确底部对齐
    setTimeout(() => {
      if (cardRef.current) {
        ...
        const bottomY = Math.max(margin, ch2 - rect.height - margin - 20) // 底部对齐，额外留20px间距
...
const hintsZH = [
 ...
 "连续做3次，自己试着完成！现在链表list1: 1->2->4, list2：3->4\n规则：🟢选择更小节点 → 接入合并链表 → 在原链表中❌删除\n完成合并链表新接3个节点",
  ...
  const hintsEN = [
    ...
    "Draw the first node of the merged list as the one you just chose, then remove it from the original list (mark with ❌).",
    "Do it three more times by yourself! Now lists are: list1: 1->2->4, list2: 3->4\nRule: 🟢 pick the smaller node → attach to merged list → ❌ delete from the original list\nFinish attaching 3 new nodes.",
    "Keep going! Merge the next node.\nBetween 4 and 4, choose one and draw the updated lists.",
    "Great! Connect the final node to finish the merged list.\nDouble-check that all nodes are included and the order is correct."
  ];
