...
export default function Home() {
  ...
  // 故事模式算法选择：迭代算法（固定）
  const [storyAlgorithm, setStoryAlgorithm] = useState<'algo1' | 'iter'>('iter');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const resp = await fetch('/initial2.excalidraw');
        if (resp.ok) {
          const data = await resp.json();
      ...
      >
        <MarkdownWithDrawing
          ...
          onAlgorithmSelect={async (alg) => {
            // 迭代页面固定使用迭代算法，忽略传入的算法选择
            if (mode === 'story') {
              await resetStoryForAlgorithm('iter');
            }
          }}
