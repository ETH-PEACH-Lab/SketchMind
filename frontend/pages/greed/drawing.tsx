...
export default function Home() {
  ...
  const hints_greed = [
    "把数组画成一排方格，下标标记在下方。\n初始化最远可达边界 🚩farthest=0，将可到达区间 [0] 涂成🟨，把当前指针 i 放到 0",
    "当前 i=0 在🟨里，最多可走步骤 reach=0+3=3，最远可达边界 🚩 应该移到哪里请标注，可达区间🟨扩到哪里请连续涂出来。",
    "继续 i 往前移，计算可走步骤，更新最远可达边界🚩和区间🟨 。一直到i到可达区间的最右边，即 i=3 。",
  ...
  // 初始 step：仅第1步从 public/initial2.excalidraw 初始化，其余空白
  useEffect(() => {
    if (!excalidrawAPI) return;
    console.log('🚀 初始化画布和场景（第1步载入 initial2.excalidraw，其余空白）');
    (async () => {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          console.log('✅ 载入 initial2.excalidraw 成功，元素数:', elements.length);
        } else {
          console.warn('⚠️ 载入 initial2.excalidraw 失败:', resp.status);
        }
      } catch (e) {
        console.warn('⚠️ 载入 initial2.excalidraw 异常:', e);
      }
    ...
    const initialScenes: Record<number, StepScene> = {};
      ...
      if (initialStep0) {
        initialScenes[0] = initialStep0;
        console.log('✅ 步骤 0 使用 initial2.excalidraw 初始化');
      } else {
        initialScenes[0] = { elements: [], files: {}, appState: { viewBackgroundColor: '#fff' } };
        console.log('✅ 步骤 0 初始化为空白画布（未找到 initial2.excalidraw）');
      }
      // 其余步骤空白
...
给你一个非负整数数组 \`nums\`，你最初位于数组的第一个下标。数组中的每个元素 \`nums[i]\` 表示在该位置可以跳跃的**最大**长度。
判断是否能够到达最后一个下标；如果可以，返回 \`true\`，否则返回 \`false\`。
...
若某个位置 \`x\` 可达，则从 \`x\` 能“一口气”覆盖一段连续区间 \`(x, x + nums[x]]\`。我们只需在遍历数组时，**维护当前能到达的最远位置 \`farthest\`**。当下标 \`i\` 不超过 \`farthest\` 时，\`i\` 可达，进而用 \`i + nums[i]\` 去拓展 \`farthest\`。一旦 \`farthest\` 覆盖了最后一个下标，就已经可以返回 \`true\`。

  ### 算法

1. 初始化 \`farthest = 0\`。
2. 线性遍历每个下标 \`i\`：
   - 若 \`i > farthest\`，说明 \`i\` 不可达，也就无法再向前推进，返回 \`false\`。
   - 否则更新 \`farthest = max(farthest, i + nums[i])\`。
   - 如果 \`farthest >= n - 1\`（\`n\` 为数组长度），立即返回 \`true\`。
3. 遍历结束后仍未覆盖末尾，返回 \`false\`。
      ...
      >
        <MarkdownWithDrawing
          ...
          onAlgorithmSelect={async (alg) => {
            ...
            if (mode === 'story') {
              await resetStoryForAlgorithm('greed');
            }
          }}
